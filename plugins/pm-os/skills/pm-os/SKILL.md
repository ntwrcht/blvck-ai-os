...
No vault found (no `ABOUT-ME/` and no `pm-os.config.md`): say so, offer `/pm-os:setup`, and fall back to the bundled defaults in `references/voice.md` and `references/frameworks.md` for one-off work.
