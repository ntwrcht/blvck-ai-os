...
**Detect mode first.** If `ABOUT-ME/CLAUDE.md` or `pm-os.config.json` exists: say "Found an existing PM OS — I'll validate it and fill gaps", run the `/blvck-pm:validate` logic, and only interview for the missing sections. Never overwrite existing content without explicit approval.

This is also the **upgrade path**: a vault built before 1.3.0 has no `vision.md`, no `language`, and no `completeness`; one built before 1.4.0 also has no `roadmap.json`; one built before 2.0.0 has `pm-os.config.md`, which is no longer read at all — convert it with `create-vault.mjs --upgrade-config --target .` and tell the user what the conversion could not carry over (integrations and the roster). Add what is missing, leave everything else untouched, and list what you added. There is no separate upgrade command by design — this one already knows how to fill gaps.
...
- `pm-os.config.json` ← the only config: paths (including vision and roadmap), `language`, an empty `completeness`, `decisions`, integrations, agent roster. As of 2.0.0 there is no markdown copy — two sources of the same truth drift silently. Field meanings: `references/config.md`
